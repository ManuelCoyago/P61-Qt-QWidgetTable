    ...
    , ui(new Ui::Principal)
{
    ...
    QStringList titulo;
    titulo << "Nombre" << "Apellido" << "Teléfono" << "E-mail" << "Nacimiento";
...
void Principal::on_btnAgregar_clicked()
{
    ...
    ui->tblLista->setItem(fila, EDAD, new QTableWidgetItem(p->edad()));
}
...
void Principal::on_btnGuardar_clicked()
{
    ...
    if (archivo.open(QFile::WriteOnly | QFile::Truncate)) {
        ...
        for (int i=0; i<filas; i++) {
            ...
            QTableWidgetItem *edad = ui->tblLista->item(i, EDAD);
            salida << nombre->text() << ";" << apellido->text() << ";";
            salida << telefono->text() << ";" << email->text() << ";" << edad->text() << "\n";
        }
        archivo.close();
...
void Principal::cargarContactos()
{
    ...
    if (archivo.open(QFile::ReadOnly)) {
        ...
        while(!entrada.atEnd()){
            ...
            ui->tblLista->setItem(fila, EDAD, new QTableWidgetItem(datos[EDAD]));
        }
        archivo.close();
    }
}
